...
$getAllIndividual = "SELECT DATE_ADD(a.`DateVaccination`, INTERVAL 28 DAY)  AS 2ndDoseSchedule, a.entity_no, category, SubPriority,CONCAT(Firstname,' ',Middlename,' ',Lastname) AS fullname, suffix, contact_no, region, province, muncity, 
...
(SELECT entity_no FROM tbl_assessment WHERE 1stDose = '01_Yes' AND 2ndDose = '01_Yes') AND a.status = 'VACCINATED' AND a.`VaccineManufacturer` = 'Sinovac' ORDER BY 2ndDoseSchedule ASC LIMIT ".$requestData['start']." ,".$requestData['length']." ";
...
$getrecordstmt->execute() or die("search_2nddose.php");

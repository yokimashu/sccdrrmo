...
if (!isset($_POST['searchTerm'])) {
  $stmt = $con->prepare("SELECT * FROM tbl_sources_infection order by patient_lastname LIMIT :limit");
  ...
  $stmt = $con->prepare("SELECT * FROM tbl_sources_infection where patient_fullname like :name order by patient_lastname LIMIT :limit");
...
foreach ($usersList as $user) {
  $response[] = array(
    "id" => $user['patient_entityno'],
    "text" => $user['patient_fullname']
  );
}

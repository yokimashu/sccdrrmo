...
$sql = "SELECT DISTINCT s.entity_no, s.cbcr, v.Firstname, v.Middlename, v.Lastname FROM tbl_schedule s inner join tbl_vaccine v on v.entity_no = s.entity_no GROUP BY s.entity_no LIMIT " . $requestData['start'] . "," . $requestData['length'] . "";
...
$sql = "SELECT DISTINCT s.entity_no, s.cbcr, v.Firstname, v.Middlename, v.Lastname FROM tbl_schedule s inner join tbl_vaccine v on v.entity_no = s.entity_no where";
...
if (!empty($requestData['search']['value'])) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
	...
	$sql .= " OR s.cbcr LIKE '%" . $requestData['search']['value'] . "%' ";
	// $sql .= " OR s.date LIKE '%" . $requestData['search']['value'] . "%' ";
	// $sql .= " OR s.time LIKE '%" . $requestData['search']['value'] . "%' ";
	// $sql .= " OR s.remarks LIKE '%" . $requestData['search']['value'] . "%' ";
	...
	$sql .= " GROUP BY s.entity_no LIMIT " . $requestData['start'] . "," . $requestData['length'] . " ";
	$get_user_data = $con->prepare($sql);
	...
	$countfilter = "SELECT COUNT(s.entity_no) as id FROM tbl_schedule s INNER JOIN tbl_vaccine v ON v.entity_no = s.entity_no where ";
	...
	$countfilter .= " OR s.cbcr LIKE '%" . $requestData['search']['value'] . "%' ";
	// $countfilter .= " OR s.date LIKE '%" . $requestData['search']['value'] . "%' ";
	// $countfilter .= " OR s.time LIKE '%" . $requestData['search']['value'] . "%' ";
	// $countfilter .= " OR s.remarks LIKE '%" . $requestData['search']['value'] . "%' ";

...
if ($traceNoSQLExecute->rowCount() > 0) {
    ...
    if ($type == "INDIVIDUAL") {
        ...
        $sql = "SELECT t.entity_no, e.type, t.fullname, t.gender, t.birthdate, t.photo,
        ...
        // check if user has table assessment
        if ($sql_data->rowCount() > 0) {
            // if true
            while ($result2 = $sql_data->fetch(PDO::FETCH_ASSOC)) {
                $all = $result2;
            }
        } else {
            // if false
        ...
        echo json_encode('QR Code is not individual!');
    }
} else {
